...
var (
	zkConnRetryIntervalMillisecs = flag.Int(
		"zk_conn_retry_interval_millisecs",
		500,
		"For non-IO operations, interval after which connection to zookeeper should be retried.")

	zkConnRetryForIoIntervalMillisecs = flag.Int(
		"zk_conn_retry_for_io_interval_millisecs",
		500,
		"For IO operations, interval after which connection to zookeeper should be retried.")
)
...
var DefaultLogger Logger = utcLogger()
...
const WAIT_TIME_FOR_SOCKET_CONNECT = 2000
...
type HostProvider interface {
	...
	// ReInit is called, when the servers need to be resolved again.
	ReInit()
...
// Return ZkConnRetyInterval in seconds.
func GetZkConnRetryInterval() int {
	return *zkConnRetryIntervalMillisecs * 1000
}
...
func ConnectWithSessionIdPassword(servers []string, sessionTimeout time.Duration, sessionId int64, passwd [16]byte) (*Conn, <-chan Event, error) {
	return Connect(servers, sessionTimeout, WithSessionIdPassword(sessionId, passwd))
}
...
func Connect(servers []string, sessionTimeout time.Duration, options ...connOption) (*Conn, <-chan Event, error) {
	...
	go func() {
		...
		conn.logger.Printf("Closing session: %d", conn.SessionID())
...
// WithSessionIdPassword returns a connection option with session id.
func WithSessionIdPassword(sessionId int64, passwd [16]byte) connOption {
	return func(c *Conn) {
...
// Return the Zookeeper client info populated with the name of the process
// using the Zookeeper go client.
func getClientInfo() []byte {
	ClientInfo := &ZookeeperClientInfo{}
	name := "UnknownGoClient"
	if (len(os.Args) > 0) && (len(os.Args[0]) != 0) {
		name = path.Base(os.Args[0])
	}
	DefaultLogger.Printf("%s is attempting to connect to Zookeeper", name)
	ClientInfo.ProcessName = &name
	data, err := proto.Marshal(ClientInfo)
	if err != nil {
		DefaultLogger.Printf("Failed to marshal client info proto to string: %s ",
			data)
		return []byte("Unknown")
	}
	return data
...
// Password returns the current password of the connection.
func (c *Conn) Password() []byte {
	return c.passwd
}
...
func (c *Conn) loop() {
	for {
		...
		c.logger.Printf("Authenticating connection 0x%x", c.SessionID())
		err := c.authenticate()
		switch {
		case err == ErrSessionExpired:
			c.logger.Printf("Authentication failed for 0x%x: %s",
				c.SessionID(), err)
		...
		case err != nil && c.conn != nil:
			...
			c.conn.Close()
		case err == nil:
			c.logger.Printf("Authenticated: id=0x%x, timeout=%d",
				c.SessionID(), c.sessionTimeoutMs)
...
func (c *Conn) authenticate() error {
	...
	// Lower the socket timeout so the client has a chance to reconnect to a different
	// Zookeeper server before the session is expired. Also inline with the Zookeeper c client
	c.conn.SetWriteDeadline(time.Now().Add(time.Duration(WAIT_TIME_FOR_SOCKET_CONNECT) * time.Millisecond))
	...
	c.conn.SetReadDeadline(time.Now().Add(time.Duration(WAIT_TIME_FOR_SOCKET_CONNECT) * time.Millisecond))
...
func (c *Conn) recvLoop(conn net.Conn) error {
	...
	for {
		...
		// Update the last zxid seen by the client.
		if res.Zxid > 0 {
			if c.lastZxid > res.Zxid {
				c.logger.Printf("Currently seen zxid (%d) greater than new zxid (%d)", c.lastZxid, res.Zxid)
			}
			c.lastZxid = res.Zxid
		}
...
func (c *Conn) ChildrenWithBackoff(path string) ([]string, *Stat, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		data, stats, err = c.Children(path)

		// Keep retrying only for the following 2 errors.
		if !(err == ErrNoServer || err == ErrConnectionClosed) {
			break
		}

		timeLapsed := time.Since(timeAtBeginning)
		if timeLapsed > time.Duration(c.sessionTimeoutMs/1000)*time.Second {
...
func (c *Conn) ChildrenWatchWithBackoff(path string) ([]string, *Stat, <-chan Event, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		data, stats, ech, err = c.ChildrenW(path)
		// Keep retrying only for the following 2 errors.
		if !(err == ErrNoServer || err == ErrConnectionClosed) {
			break
		}
...
func (c *Conn) GetWithBackoff(path string) ([]byte, *Stat, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
...
func (c *Conn) GetWatchWithBackoff(path string) ([]byte, *Stat, <-chan Event, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		data, stats, ech, err = c.GetW(path)
...
// This function will retry to do a set operation, if the previous failure is not
// due to server error or closed connection error. The parameters are path to be
// set, date to be sent, and NOS version.
func (c *Conn) SetWithBackoff(path string, data []byte, version int32) (*Stat, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		stats, err = c.Set(path, data, version)
		// Keep retrying only for the following 2 errors.
		if !(err == ErrNoServer || err == ErrConnectionClosed) {
			break
		}
...
// This function will retry to do a create operation, if the previous failure is not
// due to server error or closed connection error. The parameters are path, data to
// be created, flags, and acl for create operation.
func (c *Conn) CreateWithBackoff(path string, data []byte, flags int32, acl []ACL) (string, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		resPath, err = c.Create(path, data, flags, acl)
		// Keep retrying only for the following 2 errors.
		if !(err == ErrNoServer || err == ErrConnectionClosed) {
			break
		}
...
// This function will retry to do a delete operation, if the previous failure is not
// due to server error or closed connection error. The parameters are path and version
// number for delete operation.
func (c *Conn) DeleteWithBackoff(path string, version int32) error {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryForIoIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
...
// This function will retry to do a checking for exist operation, if the previous
// failure is not due to server error or closed connection error.
// The parameter is path to be verified.
func (c *Conn) ExistsWithBackoff(path string) (bool, *Stat, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		exists, stats, err = c.Exists(path)
...
func (c *Conn) ExistsWatchWithBackoff(path string) (bool, *Stat, <-chan Event, error) {
	// Keep retrying till session timeout if Server is not reachable.
	cb := misc.NewConstantBackoff(
		time.Duration(*zkConnRetryIntervalMillisecs)*time.Millisecond,
		misc.UnlimitedRetry)
	...
	for {
		exists, stats, ech, err = c.ExistsW(path)
