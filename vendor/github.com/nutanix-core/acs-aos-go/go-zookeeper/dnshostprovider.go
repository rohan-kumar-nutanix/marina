...
// DNSLogger uses the stdlib log package for logging.
var DNSLogger Logger = utcLogger()
...
func (hp *DNSHostProvider) Init(servers []string) error {
        ...
        hp.hostnames = servers // Keep a copy of the hostnames for the future.
        hp.servers = found
        hp.curr = -1
        hp.last = -1
...
func (hp *DNSHostProvider) ReInit() {
        hp.mu.Lock()
        defer hp.mu.Unlock()
        ...
        found := []string{}
        for _, server := range hp.hostnames {
                host, port, err := net.SplitHostPort(server)
                if err != nil {
                        return
                }
        ...
        if len(found) == 0 {
                DNSLogger.Printf("Could not resolve hostnames %s", hp.hostnames)
                return
        }
